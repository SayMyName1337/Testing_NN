...
class NoisePreprocessor:
    ...
    def preprocess_impulse_noise(self, X):
        ...
        try:
            ...
            for i in range(X_processed.shape[1]):
                ...
                # Если обнаружены выбросы, применяем медианный фильтр локально
                if np.any(outlier_mask) and window_size > 2:
                    ...
                    if np.any(expanded_mask):
                        filtered_region = median_filter(feature[expanded_mask], size=window_size)
                        filtered[expanded_mask] = filtered_region
    ...
    def preprocess_data(self, X, noise_type):
        ...
        if np.any(np.isnan(X)):
            X_processed = self.preprocess_missing_values(X_processed)
        
        # Определяем характеристики данных для адаптивной предобработки
        data_stats = self._analyze_data_characteristics(X_processed)
        
        # Удаляем выбросы (для всех типов шума)
        if noise_type != 'missing':  # Пропущенные значения обрабатываются отдельно
            X_processed = self._handle_outliers(X_processed, data_stats)
        
        # Применяем продвинутое шумоподавление на основе вейвлетов для всех данных
        # независимо от типа шума, это может значительно повысить точность
        X_processed = self.advanced_noise_reduction(X_processed)
        ...
        if noise_type in preprocessing_methods:
            X_processed = preprocessing_methods[noise_type](X_processed)
        else:
            print(f"Предупреждение: Предобработка для шума типа '{noise_type}' не реализована, используется общее шумоподавление.")
        
        # Финальная проверка на аномальные значения
        X_processed = self._final_cleanup(X_processed, data_stats)
